...
strctControllers = fnAddTextSliderEditComboSmall(strctControllers, 40+30*2, ...
	'Grating Size (Pix):', 'GratingSize', 2, min(screen_size), [1 10], fnTsGetVar(g_strctParadigm.m_strctStimulusParams, 'GratingSizePix'));
strctControllers = fnAddTextSliderEditComboSmall(strctControllers, 40+30*3, ...
	'Spatial Frequency (Hz):', 'SpatialFreq', 0.001, 0.5, [0.005, 0.01], fnTsGetVar(g_strctParadigm.m_strctStimulusParams, 'SpatialFreqHz'));
strctControllers = fnAddTextSliderEditComboSmall(strctControllers, 40+30*4, ...
	'Temporal Frequency (Hz):', 'TemporalFreq', 0.001, 20, [0.1, 1], fnTsGetVar(g_strctParadigm.m_strctStimulusParams, 'TempFreqHz'));
...
strctControllers = fnAddTextSliderEditComboSmall(strctControllers, 40+30*7, ...
	'Stimulus On Time (MS):', 'StimulusOnTime', 10, 100000, [1, 50], fnTsGetVar(g_strctParadigm.m_strctStimulusParams, 'StimulusOnMS'));
